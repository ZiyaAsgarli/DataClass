...
> This file preserves the chronological implementation record. Statements tied to an earlier numbered step describe that checkpoint, not the current production state. For current release truth, use [V1 release inventory](docs/RELEASE_V1.md), [Known issues](docs/KNOWN_ISSUES.md), and [Release status](docs/RELEASE_STATUS.md).
...
- **Completed:** Steps 13–15.1 — production deployment, UI/accessibility/date fixes, and final non-storage QA; V1.0 release readiness is documented separately
...
- **Step 6B status:** migration `0007_lesson_resources.sql`, Worker signing endpoints, and teacher/student resource UI were validated on `dataclass-step-6b`. The migration was applied to production with matching columns, constraints, indexes, functions, grants, policies, and RLS state. Development E2E validated authenticated upload intent, direct PUT, HeadObject finalization, ready metadata, authorized downloads, student visibility, and delete compatibility. At that checkpoint the Worker and production CORS remained deferred; the current production integration is recorded in `docs/STORAGE.md`.
...
- **Step 7B database boundary:** no schema defect was found, no migration was created, and production remained unchanged. Worker deployment and production-origin configuration were still deferred at that checkpoint.
...
- **Step 9.1 visual foundation:** Lumina/Stitch was used only as a visual reference while existing DataClass functionality, routes, services, and real data remained the product source of truth. The authenticated workspace now uses Hanken Grotesk headings, Inter UI text, unified warm-light and premium-dark tokens, a refined shell/navigation system, responsive real-data dashboards and assignment views, lifecycle learning-path styling, contextual Help, shared status treatments, and harmonized forms and empty states. Desktop and responsive production UI QA passed.
...
- **Step 11 localization:** the frontend uses bundled, synchronous `i18next` / `react-i18next` resources with Azerbaijani as the default and English as the secondary language. The browser-local `dataclass-language` preference is resolved before React renders, synchronizes `document.documentElement.lang`, and stays independent of authentication initialization. System UI, centralized enum/status labels, deterministic Azerbaijani month names, validation, accessibility text, Help, and teacher/student workflows are localized; user-entered course content, names, feedback, and filenames are never auto-translated. There is no database language preference, migration, backend contract change, or production change. Bilingual and responsive production QA passed.
- **Step 12 branding:** the official DataClass horizontal logo and compact DC/book/data mark are integrated as separate transparent Light/Dark raster assets without changing the established forest/sage product palette or translating the DataClass name. Manual Light/Dark and responsive production QA passed. No clean vector master currently exists, and a custom favicon remains intentionally deferred until a designer-approved simplified mark is available.
...
- **Current release boundary:** production application and product QA pass at `https://dataclass-two.vercel.app`. Public V1.0 release remains on hold for the external Neon identity-context decision, frozen XLSX reconciliation, one final real Lesson 1 XLSX E2E, and final approval.
