...
export function Sidebar({ role, items, collapsed, mobileOpen, onCollapse, onClose }: SidebarProps) {
  return (
    <>
      ...
      <aside className={cn('fixed inset-y-0 left-0 z-50 flex w-64 flex-col border-r bg-card transition-[width,transform] duration-200 lg:z-30', collapsed && 'lg:w-[76px]', mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0')}>
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto p-3" aria-label={`${role} navigation`}>
          ...
          {items.map((item) => {
            ...
            if (!item.href) return (
              <button key={item.label} type="button" disabled title={`${item.label} will be available in a later step`} className="flex h-10 w-full cursor-not-allowed items-center gap-3 rounded-lg px-3 text-sm font-medium text-muted-foreground/65" aria-label={`${item.label}, coming later`}>
        ...
        <div className="border-t p-3">
          <div className={cn('flex items-center gap-3 rounded-lg bg-muted/60 p-2.5', collapsed && 'justify-center')}>
            ...
            {!collapsed && <div className="min-w-0"><p className="truncate text-xs font-semibold">Data Analytics</p><p className="truncate text-[11px] text-muted-foreground">Professional program</p></div>}
          </div>
        </div>
