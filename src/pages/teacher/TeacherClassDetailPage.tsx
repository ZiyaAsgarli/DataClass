...
export function TeacherClassDetailPage() {
  ...
  if (error || !data) return <AppShell role="teacher" title="Class"><ErrorState retry={() => void reload()} message={dataErrorMessage(error, 'You do not have access to this class.', 'The class workspace could not be loaded. Please try again.')} /></AppShell>
  ...
  <section className="animate-enter-delay mt-8"><div className="mb-4 flex items-end justify-between gap-4"><div><h2 className="text-lg font-semibold">Modules</h2><p className="text-sm text-muted-foreground">Course sections and their classroom lessons.</p></div>{owner && <Button size="sm" onClick={() => setCreatingModule(true)}><Plus />Add module</Button>}</div>{modules.length ? <div className="grid gap-4 md:grid-cols-2">{modules.map((module, index) => <Card key={module.id} className="p-5"><div className="flex items-start justify-between gap-4"><span className="flex size-10 items-center justify-center rounded-lg bg-accent text-primary"><BookOpen className="size-5" /></span><div className="flex items-center gap-1"><Badge className="capitalize">{module.status}</Badge>{owner && <><Button size="icon" variant="ghost" disabled={index === 0} aria-label="Move module up" onClick={async () => { await reorderModule(module.id, 'up'); await reload() }}><ArrowUp /></Button><Button size="icon" variant="ghost" disabled={index === modules.length - 1} aria-label="Move module down" onClick={async () => { await reorderModule(module.id, 'down'); await reload() }}><ArrowDown /></Button></>}</div></div><h3 className="mt-5 text-lg font-semibold">{module.title}</h3><p className="mt-2 line-clamp-2 min-h-10 text-sm leading-5 text-muted-foreground">{module.description || 'No module description has been added.'}</p><div className="mt-4 space-y-1 text-xs text-muted-foreground"><p>{module.instructorNames.length ? `Instructors: ${module.instructorNames.join(', ')}` : 'Instructor: Not assigned'}</p><p>{module.lessonCount} {module.lessonCount === 1 ? 'lesson' : 'lessons'} · {module.publishedLessonCount} published</p></div><Button className="mt-5 w-full" variant="outline" asChild><Link to={`/teacher/classes/${classId}/modules/${module.id}`}>Manage module</Link></Button></Card>)}</div> : <Card className="flex min-h-56 flex-col items-center justify-center p-8 text-center"><span className="flex size-12 items-center justify-center rounded-xl bg-accent text-primary"><BookOpen /></span><h3 className="mt-4 font-semibold">No modules yet</h3><p className="mt-2 text-sm text-muted-foreground">Create the first module for this class.</p>{owner && <Button className="mt-5" onClick={() => setCreatingModule(true)}><Plus />Create module</Button>}</Card>}</section>
  ...
  {creatingModule && <ModuleFormDialog onClose={() => setCreatingModule(false)} onSave={async (title, description) => { await createModule(classId, title, description); setCreatingModule(false); await reload() }} />}
