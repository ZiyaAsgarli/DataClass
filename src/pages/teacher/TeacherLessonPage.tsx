...
import { TeacherLessonResources } from '@/components/common/LessonResources'
...
export function TeacherLessonPage() {
  ...
  return <AppShell role="teacher" title="Lesson"><Button variant="ghost" size="sm" asChild className="-ml-2 mb-5"><Link to={`/teacher/classes/${classId}/modules/${moduleId}`}><ArrowLeft />Back to module</Link></Button><Card className="animate-enter p-5 sm:p-7"><div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex items-center gap-2"><StatusBadge status={lesson.status} label={lesson.status[0].toUpperCase() + lesson.status.slice(1)} /><span className="text-xs text-muted-foreground">Lesson {String(lesson.position + 1).padStart(2, '0')}</span></div><h1 className="mt-4 text-3xl font-semibold tracking-[-0.035em] sm:text-4xl">{lesson.title}</h1><p className="mt-2 text-sm text-muted-foreground">{lesson.moduleTitle} · {lesson.className}</p></div>{canManage && <Button variant="outline" onClick={() => setEditing(true)}><Edit3 />Edit lesson</Button>}</div><p className="mt-6 flex items-center gap-2 text-sm text-muted-foreground"><CalendarDays className="size-4" />{lesson.lessonDate || 'Lesson date not set'}</p></Card>
  ...
  <Card className="mt-6 p-5 sm:p-7"><h2 className="font-semibold">Lesson details</h2><p className="mt-4 max-w-3xl whitespace-pre-wrap text-sm leading-7">{lesson.description || 'No lesson description has been added.'}</p></Card><TeacherLessonResources lessonId={lesson.id} canManage={canManage} />
