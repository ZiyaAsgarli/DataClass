...
export function StudentLessonPage() {
  ...
  return <AppShell role="student" title="Lesson"><Button variant="ghost" size="sm" asChild className="-ml-2 mb-5"><Link to={`/student/classes/${classId}/modules/${moduleId}`}><ArrowLeft />Back to module</Link></Button><Card className="animate-enter p-5 sm:p-7"><div className="flex flex-wrap items-center gap-2"><StatusBadge status="published" label="Published" /><span className="text-xs text-muted-foreground">Lesson {String(lesson.position + 1).padStart(2, '0')}</span></div><h1 className="mt-4 text-3xl font-semibold tracking-[-0.035em] sm:text-4xl">{lesson.title}</h1><p className="mt-2 text-sm text-muted-foreground">{lesson.moduleTitle} · {lesson.className}</p><p className="mt-5 flex items-center gap-2 text-sm text-muted-foreground"><CalendarDays className="size-4" />{lesson.lessonDate || 'Lesson date not set'}</p></Card>
