...
export function StudentAssignmentDetailPage() {
  ...
  if (loading)
    return (
      <AppShell role="student" title="Assignment">
  ...
  if (error || !data)
    return (
      <AppShell role="student" title="Assignment">
        <ErrorState retry={() => void reload()} message={dataErrorMessage(error, 'This assignment is not available to you.', 'The assignment could not be loaded. Please try again.')} />
      </AppShell>
    )
    ...
    <AppShell role="student" title="Assignment">
      ...
      <Card className="p-5 sm:p-7">
        ...
        <p className="mt-5 flex items-center gap-2 text-sm text-muted-foreground">
          ...
          {formatDateTime(assignment.dueAt)} · {assignment.allowLateSubmission ? 'Late submissions allowed' : 'Late submissions close at the deadline'}
        </p>
      </Card>
      <Card className="mt-6 p-5 sm:p-7">
        <h2 className="font-semibold">Instructions</h2>
        <p className="mt-4 whitespace-pre-wrap text-sm leading-7">{assignment.description || 'No additional instructions were provided.'}</p>
      </Card>
      <div className="mt-6">
      ...
      <Card className="mt-6 p-5 sm:p-7">
        <div className="flex items-center gap-2">
          {status === 'reviewed' ? <CheckCircle2 className="size-5 text-primary" /> : status === 'revision_requested' ? <RotateCcw className="size-5 text-amber-600" /> : <Clock3 className="size-5 text-muted-foreground" />}
          <h2 className="font-semibold">Your submission</h2>
        </div>
        {assignment.feedbackMessage && (
          <div className="mt-5 rounded-lg border bg-muted/30 p-4">
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">Teacher feedback</p>
            <p className="mt-2 whitespace-pre-wrap text-sm leading-6">{assignment.feedbackMessage}</p>
          </div>
        )}
        {files.length > 0 && (
          <div className="mt-5">
        ...
        {canUpload ? (
          ...
          <p className="mt-5 text-sm text-muted-foreground">{status === 'reviewed' ? `Reviewed ${formatDateTime(assignment.reviewedAt, 'time unavailable')}` : `Submitted ${formatDateTime(assignment.submittedAt, 'time unavailable')}`}</p>
        )}
      </Card>
    </AppShell>
  )
