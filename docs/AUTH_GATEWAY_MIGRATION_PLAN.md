...
- Set transaction-local `statement_timeout = 10s` and `idle_in_transaction_session_timeout = 15s`; use a conservative `lock_timeout = 3s` for mutations after Step 16.3B verifies current locks.
- Keep the live-proven 12-second Worker response deadline. The deadline rejects without manipulating an in-flight socket. Dispose only a successfully connected, usable client; rollback only after confirmed `BEGIN`; cleanup never replaces the primary error.
...
## Blockers before 16.3B

There is no architecture blocker to starting isolated SQL work. Step 16.3B must first freeze the generated 84-function/9-policy manifest as a test fixture, decide exact capability-role table commands and non-recursive RLS policies, define the `app_gateway` naming/version convention, and add contract tests for all scalar/row/void/error shapes. Production credentials, identifiers, data, routes, and the frozen XLSX incident are not inputs to that step.
