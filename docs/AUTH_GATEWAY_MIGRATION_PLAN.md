...
## Step 16.5A — production cutover prep and rollback rehearsal

The feature source, production baseline, and clean working state were verified before this preparation. The frozen SQL artifact remains the approved migration: its SHA-256 is `AE5295E081E542AA7C820AE875687712BB6D7FAE39577922D54CE9FEACE66A01`. It is a single PostgreSQL transaction and still matches the 84-function manifest, 71 callable operations, unchanged public signatures, nine audited legacy RLS policies, 15 nonrecursive gateway capability policies, seven triggers, restricted LOGIN/capability-owner model, and `app_private.current_actor_id()` identity contract.

A direct read-only preflight resolved the actual production database branch from the deployed Worker configuration. All 84 application functions and exact manifest signatures were present, with no missing callable operation. The nine active legacy policies and seven triggers matched the baseline. The pre-migration `auth.uid()`/`auth.user_id()` dependency counts remained 76 functions and three direct policies, with six additional helper-dependent policies. The gateway schemas and roles were absent as expected. Production data counts were plausible, and the frozen incident remained exactly two pending XLSX rows and zero ready resources in its Lesson 1 context. No material production schema drift was found and no production statement modified state.

Production Neon currently exposes a six-hour history window and no existing named snapshot. The cutover restore plan records an exact UTC timestamp and `pg_current_wal_lsn()` immediately before SQL, optionally creates a named snapshot only under cutover authorization, and uses Neon branch point-in-time restore to that timestamp or LSN if committed SQL must be rolled back. The maintenance window must prevent or reconcile later writes because a restore discards changes after the selected point. A longer Neon restore window is recommended before commercial operation.

The production Auth structural preflight used the real supported SDK login flow and application session hydration. Source and deployed-bundle inspection confirmed `neonClient.auth.getSession()` and the transformed `data.session.token` path, with no raw session fetch or public `getJWTToken()` path. The fixed HTTPS JWKS publishes EdDSA/Ed25519 public keys only. The configured issuer/audience contract, UUID subject, expiration, and optional `nbf` match the gateway's live-proven verifier. No token or personal claim was printed, stored, or committed, and production Auth was not changed.

The production B2 key values and capabilities are intentionally unreadable from Worker configuration. Cutover therefore must verify or replace that key with one restricted exclusively to `dataclass-resources` and the least capabilities `listFiles`, `readFiles`, `writeFiles`, and `deleteFiles`. `listFiles` is required for the Native API exact-key inspection added in D4C1. The private bucket, region, endpoint, signed direct PUT/GET design, CORS, and existing objects require no migration.

The frozen deployment order is: maintenance and health check; Neon restore reference; DB precheck; checksummed SQL apply; SQL postcheck; restricted credential; Hyperdrive; Worker configuration and deploy while retaining the legacy binding; Worker Auth/gateway/storage smoke; frontend environment and deploy; teacher/student and disposable storage smoke; Data API-zero verification; legacy binding removal and Worker redeploy; repeat smoke; completion. The frontend gateway and storage URLs both resolve to the production Worker origin, while existing Neon Auth frontend configuration remains in place.

Rollback was rehearsed only in isolated infrastructure. A prior Worker version and the current version both passed explicit dry-run deployment selection. An isolated Worker config without Hyperdrive and its restored binding config both passed dry-run. Transactional revocation of the isolated gateway LOGIN was rolled back successfully, and the complete staged SQL was executed with its final commit replaced by rollback, leaving all 84 migrated functions and the LOGIN state intact. The production-baseline frontend commit installed and built as a detached rollback artifact and was then removed. No isolated live traffic was redirected and no production configuration changed.

The operational procedure, validation queries, deterministic failure cut points, credential and Hyperdrive controls, Worker/frontend inventory, smoke matrix, legacy-binding removal, point-in-time rollback, deferred frozen XLSX cleanup, Git release strategy, and capacity considerations are frozen in `docs/PRODUCTION_AUTH_GATEWAY_CUTOVER.md`. The last safe point with no production change is immediately before SQL execution. After commit, rollback uses the recorded Neon point in time plus prior Worker/frontend deployments and saved binding/environment configurations; no runtime Data API fallback or mutation replay is permitted. There is no designed irreversible point while those restore and deployment artifacts remain available, but the six-hour database history window and reconciliation of post-restore writes are material limits.

The application architecture presents no obvious technical blocker for 10 or more classes. Commercial launch requires a Vercel plan that permits commercial use, and usage must be monitored against Cloudflare Worker/Hyperdrive, Neon compute/storage/restore, and B2 storage/egress limits. Step 16.5A performs no production mutation. The remaining gate is explicit production cutover authorization after review of the runbook, restore ownership, production B2 key capability, secure gateway credential handling, and the exact merged `main` commit.
